...
PatchFaceTag::IdentifyAdaptivePatches(
    ...
    for (int levelIndex = 0; levelIndex < refiner.GetNumLevels(); ++levelIndex) {
        ...
        for (int faceIndex = 0; faceIndex < level->getNumFaces(); ++faceIndex) {
            ...
            patchTag.hasPatch  = true;
            patchTag.isRegular = not hasXOrdinaryVertex or hasNonManifoldVertex;

            // single crease patch optimization
            if (useSingleCreasePatch and
            ...
            if (approxSmoothCornerWithRegularPatch) {
                if (!patchTag.isRegular and (patchTag.boundaryCount == 2)) {
