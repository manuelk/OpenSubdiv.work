...
public:
    ...
    // patch weights
    static void GetPatchWeights(PatchParam const & param,
        float sharpness, float s, float t, float point[], float deriv1[], float deriv2[]);
...
template <SplineBasis BASIS>
void Spline<BASIS>::GetPatchWeights(PatchParam const & param,
    float sharpness, float s, float t, float point[16], float derivS[16], float derivT[16]) {
...
void GetBSplineWeights(PatchParam const & param,
    float sharpness, float s, float t, float point[16], float deriv1[16], float deriv2[16]) {

    Spline<BASIS_BSPLINE>::GetPatchWeights(param, sharpness, s, t, point, deriv1, deriv2);
}
